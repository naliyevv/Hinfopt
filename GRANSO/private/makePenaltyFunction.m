...
% =========================================================================
    ...
    if ~isa(obj_fn, 'function_handle')
        error('GRANSO:userSuppliedFunctionsError','obj_fn must be supplied.');
    end
    ...
    function f_u = getUnscaledFValue()
        f_u             = unscaleValues(f,scaling_f);
    end
    ...
    function unscaled = getUnscaledField()
        unscaled = dataStruct(x,getUnscaledFValue());
    end

    function unscaled = getUnscaledFieldConstrained()
        f_u         = unscaleValues(f,scaling_f);
        ci_u        = unscaleValues(ci,scaling_ci);
        ce_u        = unscaleValues(ce,scaling_ce);
        ...
        unscaled    = dataStructConstrained(                            ...
                                x, f_u, ci_u, ce_u, tvi_u, tve_u, tv_u, ...
                                is_feasible_to_tol_fn(tvi_u,tve_u), mu  );
    end
    
    function [soln, stat_value_o] = getBest()
        [scalings_field,unscaled_field] = getUnscaledFields();
    ...
    function [soln, stat_value_o] = getBestConstrained()
        final               = getInfoForXConstrained();
        [scalings_field,unscaled_field] = getUnscaledFields();
        if isempty(best_to_tol)
            to_tol_field    = {};
        else
            to_tol_field    = {'best_to_tol', best_to_tol};            
        end
        soln = struct(  scalings_field{:},                      ...
                        unscaled_field{:},                      ...
                        'final',            final,              ...               
    ...
    function [scalings_field,unscaled_field] = getUnscaledFields()
        if prescaled
            unscaled        = get_unscaled_fn();
            unscaled_field  = {'final_unscaled',unscaled};
