...
%       .unscaled 
%           Print the unscaled values of the functions, if prescaling was
%           used.  Requires a struct of the unscaled value, which is
%           provided by the makePenaltyFunction object.
%       .regularizeError
...
% =========================================================================
    ...
    function unscaled(unscaled_at_x)   
        ...
        viol_values = viol_vals_fn(unscaled_at_x); 
                        
        table_printer.row(  iter_c.format_fn(last_iter),                ...
                            pen_values{1},                              ...
                            pen_c.na_str,                               ...
                            obj_c.format_fn(unscaled_at_x.f),           ...
                            viol_values{:},                             ...
                            info_values{:},                             ...
                            stat_c.na_str                               );
    end
    ...
    function lineSearchRestart(mu_ls)
        msg = sprintf([ 'Line search failed to bracket a minimizer, '   ...
                        'reattempting with mu = %g.'], mu_ls            );
        table_printer.msg(msg); 
    end

    function printBfgsInfo(code)             
        switch code
            case 1
        ...
        table_printer.msg(['BFGS update ' r]);
    end

    function printRegularizeError()
        msg = 'Regularization skipped due to eig() error.';
        table_printer.msgOrange(msg);
    end

    function printQPError(err,loc)
   
        % title to appear in top/bottom borders
        t1 = sprintf('GRANSO: QUADPROG ERROR IN %s QP - START',loc);
    ...
    function gransoHeader()
        ...
        table_printer.msg(nameVersionCopyrightMsg());
...
s = {                                                                   ...
''                                                                      ...
'GRANSO applied pre-scaling at x0.  The iteration info printed:'        ...
' - ABOVE shows values for the pre-scaled problem'                      ...
' - BELOW shows the unscaled values at the final iterate.'              ...
'The final unscaled values and the pre-scaling multipliers are'         ...
'respectively stored in soln.final_unscaled and soln.scalings.'         ...
