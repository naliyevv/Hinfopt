...
%
%       .final_unscaled Unscaled values of the function(s) at the last
%                       accepted iterate.
%
%       Furthermore, soln will also contain the following computed 
%       solutions, though note that if pre-scaling has been applied, these 
%       will all be with respect to the pre-scaled version of the problem
%       and may thus not be solutions to the original unscaled problem:
%
...
%
%       For final_unscaled, .final, .best, and .most_feasible, the
%       the following subsubfields are always present:
...
%
%           10: User-supplied functions returned inf/NaN at x0.
%
...
% =========================================================================
    ...
    if opts.print_level && opts.quadprog_info_msg
        print_notice_fn('QUADPROG NOTICE',quadprogInfoMsg());
    end
        
    if opts.print_level && opts.prescaling_info_msg
    ...
    if opts.print_level 
        if penaltyfn_obj.isPrescaled()
            printer.unscaled(soln.final_unscaled);
        end
        str     = getTerminationConditionString(soln,constrained);
        lines   = wrapToLines(str,printer.msgWidth(),0);
        printer.msg(lines);
        if qp_fail_rate > 1
...
function [problem_fns,options] = processArguments(n,obj_fn,varargin)
    if nargin > 3
        problem_fns     = {obj_fn, varargin{1}, varargin{2}};
        options_index   = 3;  
    else
        problem_fns     = {obj_fn};
        options_index   = 1;
...
function s = getTerminationConditionString(soln,constrained)
    switch soln.termination_code
        ...
        case 10
            s = 'user-supplied functions returned inf or NaN at initial iterate.';
        case 11
