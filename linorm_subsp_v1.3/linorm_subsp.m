...
%                           D_j, and E_j in (2), respectively. If k_x is 
...
%     using rational interpolation, IFAC-PapersOnLine, 51(25):84-89, 2018.
%     Joint 9th IFAC Symposium on Robust Control Design and 2nd IFAC
%     Workshop on Linear Parameter Varying Systems, Florianópolis, Brazil,
%     2018.
%
% AUTHORS:
%
...
% Matthias Voigt, 06/2018, 07/2018, 03/2019.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  LINORM_SUBSP 1.3 Copyright (C) 2019 Nicat Aliyev, Emre Mengi,
...
if doLoewner
    ...
    if boydbalak == 0
        ...
        %
        [ f, z, infonorm ] = linorm_h( full( pars.A ), full( pars.E ), ...
        ...
        if infonorm == 0
            ...
            if infonorm == 7 || infonorm == 8
                ...
                %
                % AB13HD.f threw an error, the MATLAB function 'norm' is
                % used as a fallback option.
                %
                parssys = dss( pars.A, pars.B, pars.C, pars.D, pars.E );
                [ f, z ] = norm( parssys, Inf, 1e-6 );
            end
        end
    else
        %
    ...
    while ( abs( z-zOld ) > tolz*( 0.5*( abs( z )+abs( zOld ) ) ) && ...
        ...
        if boydbalak == 0
            ...
            % Use AB13HD.f/linorm_h.F.
            %
            [ f, z, infonorm ] = linorm_h( full( pars.A ), ...
            ...
            if infonorm == 0
                ...
                if infonorm == 7 || infonorm == 8
                    ...
                    %
                    % AB13HD.f threw an error, the MATLAB function 'norm' 
                    % is used as a fallback option.
                    %
                    parssys = dss( pars.A, pars.B, pars.C, pars.D, ...
                        pars.E );
                    [ f, z ] = norm( parssys, Inf, 1e-6 );
                end
            end
            ...
            if boydbalak == 0
                ...
                if infonorm == 0
                    ...
                    if infonorm == 7 || infonorm == 8
                        ...
                        %
                        % AB13HD.f threw an error, the MATLAB function 
                        % 'norm' is used as a fallback option.
                        %
                        parssys = dss( pars.A, pars.B, pars.C, pars.D, ...
                            pars.E );
                        [ f, z ] = norm( parssys, Inf, 1e-6 );
        ...
        switch algo
            ...
            case 'b'
                ...
                if boydbalak == 0
                    ...
                    if infonorm == 0
                        if f( 2 ) == 0 || z( 2 ) == 0
                            error( [ 'linorm_h: The reduced function ', ...
                                'is not in L-infinity.' ] );
                        else
                            f = f( 1 )/f( 2 );
                            z = z( 1 )/z( 2 );
                        end
                        ...
                        if infonorm == 7 || infonorm == 8
                            ...
                            % 'norm' is used as a fallback option.
                            %
                            parssys = dss( pars.A, pars.B, pars.C, ...
                                pars.D, pars.E );
                            [ f, z ] = norm( parssys, Inf, 1e-6 );
                        end
                    end
