function [ f, g ] = fg_anemometer_3p( mu, sys )
%
%% PURPOSE:
%
% To evaluate the L-infinity norm of the (reduced) anemometer_3p example 
...
% where
%
%   A(μ) = A1 + μ2*A2 + μ1*μ3*A3,   μ in [0,1] x [1,2] x [0.1,2],
%   E(μ) = E1 + μ1*E2.
%
% ARGUMENTS
%
...
v = V( :, 1 );
%             
dH1 = -Cmu/( 1i*z*Emu - Amu )*( 1i*z*E2 - mu( 3 )*A3 )* ...
    ( ( 1i*z*Emu - Amu )\Bmu );
dH2 = -Cmu/( 1i*z*Emu - Amu )*( -A2 )*( ( 1i*z*Emu - Amu )\Bmu );
dH3 = -Cmu/( 1i*z*Emu - Amu )*( -mu( 1 )*A3 )*( ( 1i*z*Emu - Amu )\Bmu );
%
g( 1, 1 ) = real( u'*dH1*v );
g( 2, 1 ) = real( u'*dH2*v );
g( 3, 1 ) = real( u'*dH3*v );
