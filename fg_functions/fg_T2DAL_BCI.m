function [ f, g ] = fg_T2DAL_BCI( mu, sys )
%
%% PURPOSE:
%
...
% where
%
%   A(μ) = A4 - μ1*A1 - μ2*A2 - μ3*A3,
%                                   μ in [1,10000] x [1,10000] x [1,10000].
%
% ARGUMENTS
%
% Inputs:
...
if method == 0
    ...
    %
    [ f, z, infonorm ] = linorm_h( Amu, Emu, Bmu, Cmu, zeros( 7, 1 ), ...
        1, 0, 1, 0 );
    %
    if infonorm == 0
        if f( 2 ) == 0 || z( 2 ) == 0
        ...
        if infonorm == 7 || infonorm == 8
            ...
            %
            sysmu = dss( Amu, Bmu, Cmu, zeros( 7, 1 ), Emu );
...
dH3 = -Cmu/( 1i*z*Emu - Amu )*A3*( ( 1i*z*Emu - Amu )\Bmu );
%
g( 1, 1 ) = real( u'*dH1*v );
g( 2, 1 ) = real( u'*dH2*v );
g( 3, 1 ) = real( u'*dH3*v );
