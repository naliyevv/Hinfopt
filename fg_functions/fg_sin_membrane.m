function [ f, g ] = fg_sin_membrane( mu, sys )
%% PURPOSE:
%
% To evaluate the L-infinity norm of the (reduced) sin_membrane example 
...
%
%   A(μ) = A1 + μ2*A2 + μ4*A3,   
%   E(μ) = E1 + μ2*μ4*E2,   μ in [2,5] x [400,750] x [3000,3200] x [10,12].
%
% ARGUMENTS
%
% Inputs:
...
%    sys.C                : = C.
%    sys.E                : = {E1,E2}.             
%    sys.f.A              : = @(mu)[1,mu(1),mu(4)].
%    sys.f.B              : = @(mu)1.
%    sys.f.C              : = @(mu)1.
%    sys.f.E              : = @(mu)[1,mu(2)*mu(3)].
...
if method == 0
    ...
    %
    [ f, z, infonorm ] = linorm_h( Amu, Emu, Bmu, Cmu, [ 0; 0 ], 1, 0, ...
        1, 0 );
    %
    if infonorm == 0
        if f( 2 ) == 0 || z( 2 ) == 0
...
v = V( :, 1 );
%             
dH1 = -Cmu/( 1i*z*Emu - Amu )*( -A2 )*( ( 1i*z*Emu - Amu )\Bmu );
dH2 = -Cmu/( 1i*z*Emu - Amu )*( 1i*mu( 3 )*E2 )*( ( 1i*z*Emu - Amu )\Bmu );
dH3 = -Cmu/( 1i*z*Emu - Amu )*( 1i*mu( 2 )*E2 )*( ( 1i*z*Emu - Amu )\Bmu );
dH4 = -Cmu/( 1i*z*Emu - Amu )*( -A3 )*( ( 1i*z*Emu - Amu )\Bmu );
%
g( 1, 1 ) = real( u'*dH1*v );
g( 2, 1 ) = real( u'*dH2*v );
g( 3, 1 ) = real( u'*dH3*v );
